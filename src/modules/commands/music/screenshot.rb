module Bot::DiscordCommands
  module Music
    ...
    command(:screenshot, requirements: [:in_voice, :playing], type: :Music, description: 'Send a screenshot of the current video.') do |event|
      system("ffmpeg -loglevel panic -y -ss #{event.voice.stream_time.to_i + Bot.masterqueue[event.server.id].first[:skipped_time]} -i #{Bot.masterqueue[event.server.id].first[:location]} -vframes 1 data/musiccache/screenshot.png")
